...
**cookieの到達性は依然として未確認。** 検証にはAccess applicationが有効な環境が必要で、その有効化はCloudflare側の操作(Zero Trustダッシュボード)を要する。手元のOAuthトークンにはZero Trust系のスコープが無く、実測できていない。

**両方の分岐を実装して、この未確認事項が所有者の閲覧を止めないようにした。**

- cookieが届く場合: `src/auth.ts` が `CF_Authorization` cookie のJWTをヘッダと同一の手順で検証する(署名・`iss`・`aud`・期限)。検証を通らないcookieは識別子として扱わないため、偽装しても非公開アーティファクトへは到達しない
- 届かない場合: 一覧画面が非公開の項目に `/_auth/view?target=/<uid>/<name>` へのリンク(「所有者として開く」)を出す。このパスはAccess保護下で `Cf-Access-Jwt-Assertion` が確実に届き、所有者判定の上で配信と同じヘッダで本文を返す

当初の分岐案にあった「非公開かつ未認証を302で `/_auth/view` へ送る」方式は採らなかった。302と404の出方で存在の有無が漏れるおそれがあり、FR-017・FR-024の「4つの404が完全に同一」という不変条件を保つ方が優先だと判断した。リンク方式なら配信パスの応答を1種類の404に保てる。

`/_auth/view` の `target` は `/<uid>/<name>` の形に限定し、`//` 始まり・スキーム付きURL・`..` を含む値・階層が深い値・名前規則に反する値を拒否する(`tests/integration/owner-view.test.ts` で10ケースを検証)。

cookieの到達性が実測できた後も、この実装は変更を要しない。届くと分かればリンクは冗長になるだけで、害はない。
