...
CSPと画面の噛み合わせは、ヘッダを読むだけでは壊れたことが分からない。管理画面のCSPかレイアウトを触ったら、ブラウザで1度だけ確認する。

1. `/_app/` と `/_app/upload` を開き、開発者ツールのコンソールを見る
   - **期待**: CSPのエラーが出ていない
2. コンソールで `document.styleSheets.length` を評価する
   - **期待**: `0` より大きい。配色・余白・テーブルの罫線・公開状態のバッジ・空状態の枠線が実際に描画されている

`npm run test:e2e` は、この前提 ―― ページがインラインの `<style>` を持たず、同一オリジンのCSSを指し、その宛先が実際に空でないCSSを返すこと ―― までを自動で確認する。ブラウザで最終的な見た目を見るのは人の仕事として残る。
